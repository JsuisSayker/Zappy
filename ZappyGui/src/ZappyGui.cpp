    ...
    if (phiras >= actualPhiras) {
        for (int i = 0; i < phiras - actualPhiras; i++) {
            ...
            map[x][y].phiras.push_back(createGameObject(
                ...
                {static_cast<float>(x) - 0.3f, z, static_cast<float>(y)},
                {0.f, 0.f, 0.f}, {1.f, 1.f, 1.f}, true));
        }
    } else {
