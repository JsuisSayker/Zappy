...
Test(gui_command_mct, test_gui_command_mct)
{
    ...
    if (pid_1 == 0) {
        // Child process
        sleep(sleep_time);
        char **commands = splitter("GRAPHIC|mct", "|");
...
Test(gui_command_mct, test_gui_command_mct_bad_cmd)
{
    // Configuration for the server
    int sleep_time = 3;
    args_config_t *args = calloc(1, sizeof(args_config_t));
    ...
    if (pid_1 == 0) {
        // Child process
        sleep(sleep_time);
        char **commands = splitter("GRAPHIC|mctaze", "|");
        client("127.0.0.1", args->port, commands, 4);
        free_array(commands);
        exit(0);
    } else {
