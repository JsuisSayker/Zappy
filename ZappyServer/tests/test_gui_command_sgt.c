...
Test(gui_command_sgt, test_gui_command_sgt)
{
    ...
    if (pid_1 == 0) {
        // Child process
        sleep(sleep_time);
        char **commands = splitter("GRAPHIC|sgt", "|");
        client("127.0.0.1", args->port, commands, 4);
        free_array(commands);
        exit(0);
    } else {
        pid_t pid_2 = fork();
...
Test(gui_command_sgt, test_gui_command_sgt_bad_command)
{
    // Configuration for the server
    int sleep_time = 3;
    args_config_t *args = calloc(1, sizeof(args_config_t));
    ...
    if (pid_1 == 0) {
        // Child process
        sleep(sleep_time);
        char **commands = splitter("GRAPHIC|sgtaze", "|");
        client("127.0.0.1", args->port, commands, 4);
        free_array(commands);
        exit(0);
    } else {
